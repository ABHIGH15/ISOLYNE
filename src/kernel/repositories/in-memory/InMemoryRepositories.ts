...
export class InMemoryGapRepository implements GapRepository {
  ...
  async save(gap: AwarenessGap) { 
    ...
    if (idx !== -1) this.gaps[idx] = gap; // Upsert
    else this.gaps.push(gap);
  }
  async getActive(squadId: string) { 
    return this.gaps.filter(g => (g as any).status !== 'resolved' && g.squadId === squadId); 
  }
...
export class InMemoryProposalRepository implements ProposalRepository {
  ...
  async save(proposal: DecisionProposal) { 
    const idx = this.proposals.findIndex(p => p.id === proposal.id);
    if (idx !== -1) this.proposals[idx] = proposal; // Upsert
